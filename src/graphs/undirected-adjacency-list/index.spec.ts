...
import {IShortestPathPayload, ITraversalPayload, ShortestPathCallback} from './types';

describe('Undirected Adjacency Graph Unit tests', () => {
  describe("Dijkstra's Shortest Path", () => {
    const callback: ShortestPathCallback<IStates, IDistance> = (vertexAndEdge: IShortestPathPayload<IStates, IDistance>) => {
      return vertexAndEdge.edge.payload.miles;
    };
    it('Should find the shortest path from Augusta to Albany', () => {
      const graph = setupGraphs();

      expect(graph.dijkstraShortestPath('Maine', 'New York', callback)).toEqual(["New Hampshire", "Vermont", "New York"]);
      expect(graph.dijkstraShortestPath('Massachusetts', 'New York', callback)).toEqual(["Connecticut", "New York"]);
    });
    ...
    it('Should throw if given nonexistant vertices', () => {
      const graph = setupGraphs();
      expect(() => graph.dijkstraShortestPath('Ramsflat', 'Maine', callback)).toThrowErrorMatchingSnapshot();
      expect(() => graph.dijkstraShortestPath('Maine', 'Farblebutt', callback)).toThrowErrorMatchingSnapshot()
    });
  });
  ...
  describe('Fetch Data from graph', () => {
    ...
    it("Should give me the payload of an vertex between two points in a graph", () => {
      const graph = setupGraphs();
      expect(graph.getEdge('Massachusetts', 'New Hampshire')).toEqual({miles: 164});
    });

    it('Should give me all the vertexes a single vertex is connected to', () => {
      const graph = setupGraphs();
      expect(graph.getEdges('Massachusetts')).toEqual(["Connecticut", "New Hampshire", "Vermont", "Rhode Island"]);
    });
